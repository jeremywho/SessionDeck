...
matching the omc status-line thresholds), and **Idle** time humanized (`42m`, `1h 5m`). **Hover a row**
for the rest — model, context tokens, status, last tool, folder, PID/Id/version (and the API-error text
when it's in that state). A footer shows the live count and a color legend.
